...
# get x axis values
x_range = pd.read_csv('/home/houtlaw/iono-net/data/SAR_AF_ML_toyDataset_etc/no_noise/meta_X_20241216_155104.csv').iloc[:,0].values

# get config dictionary
with open('/home/houtlaw/iono-net/data/SAR_AF_ML_toyDataset_etc/no_noise/setup_20241216_155105.json') as f:
    setup = json.load(f)
...
# get true point scatterer (without speckle)
scatterer_path = '/home/houtlaw/iono-net/data/SAR_AF_ML_toyDataset_etc/no_noise/nuStruct_withSpeckle_20241216_155104.csv'
...
plt.savefig('scatterers_clean.png', dpi=300)




# get noisy signal
signal_path = "/home/houtlaw/iono-net/data/SAR_AF_ML_toyDataset_etc/no_noise/uscStruct_vals_20241216_155104.csv"
...
# get kpsi values
kpsi_path = "/home/houtlaw/iono-net/data/SAR_AF_ML_toyDataset_etc/no_noise/kPsi_20241216_155104.csv"
...
psi_coeffs_path = "/home/houtlaw/iono-net/data/SAR_AF_ML_toyDataset_etc/no_noise/compl_ampls_20241216_155104.csv"
...
plt.savefig('input_signal_clean.png', dpi=300)
...
plt.savefig('image_integral_clean.png', dpi=300)




# get model output coefficients
with open('/home/houtlaw/iono-net/model/weights/model_weights_20241216_155744.pkl', 'rb') as f:
...
plt.savefig('neural_image_integral_clean.png', dpi=300)


fig = plt.figure(figsize=(30, 8))
plt.rcParams["figure.figsize"] = (100,100)
plt.plot(x_range, np.absolute(image_integral)/dx, 'red', lw = 6)
plt.plot(x_range, np.absolute(model_image_integral)/dx, 'blue', lw = 2)
plt.title("Image Integral (NN)")
plt.legend([ "Image Integral (Known)", "Image Integral (NN)"])
plt.savefig('neural_image_integral_combined_clean.png', dpi=300)


#Generate plot with sinc overlay
delta_location = x_range[np.argmax(true_scatterers)]
delta_val = np.max(true_scatterers)

image_radius = 12 # indices of x data around location of delta

zoom_image_integral =  (np.absolute(model_image_integral)/dx)[np.argmax(true_scatterers) - image_radius:np.argmax(true_scatterers)+image_radius]
zoom_x_range = x_range[np.argmax(true_scatterers) - image_radius:np.argmax(true_scatterers)+image_radius]
zoom_model_image_integral = (np.absolute(model_image_integral)/dx)[np.argmax(true_scatterers) - image_radius:np.argmax(true_scatterers)+image_radius]

fig = plt.figure(figsize=(30, 8))
plt.rcParams["figure.figsize"] = (100,100)
plt.plot(zoom_x_range, zoom_image_integral, 'red', lw = 6)
plt.plot(zoom_x_range, zoom_model_image_integral, 'blue', lw = 2)
plt.plot(zoom_x_range, (np.sinc(zoom_x_range - delta_location) * delta_val), 'orange', lw = 2)
plt.vlines(delta_location, 0, delta_val, colors='green', linestyles='dashed', lw = 2)
plt.hlines(0, np.min(zoom_x_range), np.max(zoom_x_range), colors = 'green', linestyles='dashed', lw = 2, label='_nolegend_')
plt.title("Image Integrals vs Sinc")
plt.legend([ "Image Integral (Known)", "Image Integral (NN)", "Sinc Function", "Point Scatterer"])
plt.savefig('zoomed_combined_clean.png', dpi=300)
