...
with open('/home/houtlaw/iono-net/utils/sar_objects/noise_experiments/more_speckle/setup_20241227_131829.json') as f:
    setup = json.load(f)
...
scatterer_path_speckle = '/home/houtlaw/iono-net/utils/sar_objects/noise_experiments/more_speckle/nuStruct_withSpeckle_20241227_131827.csv'
true_scatterers_speckle = pd.read_csv(scatterer_path_speckle).map(convert_to_complex).iloc[:,sample_idx].map(np.absolute).values
fig = plt.figure(figsize=(30, 8))
plt.plot(x_range, true_scatterers_speckle)
...
# get noisy signal
signal_path_speckle = "/home/houtlaw/iono-net/utils/sar_objects/noise_experiments/more_speckle/uscStruct_vals_20241227_131828.csv"
signal_df_speckle = pd.read_csv(signal_path_speckle).map(convert_to_complex).T.iloc[sample_idx,:].values
sample_signal_vals_speckle= np.vstack((x_range, signal_df_speckle))

# get kpsi values
kpsi_path = "/home/houtlaw/iono-net/utils/sar_objects/noise_experiments/more_speckle/kPsi_20241227_131829.csv"
...
psi_coeffs_path_speckle = "/home/houtlaw/iono-net/utils/sar_objects/noise_experiments/more_speckle/compl_ampls_20241227_131828.csv"
psi_coeffs_df_speckle = pd.read_csv(psi_coeffs_path_speckle).T
for col in psi_coeffs_df_speckle.columns:
    # Replace 'i' with 'j' for Python's complex number format and convert to complex numbers
    psi_coeffs_df_speckle[col] = psi_coeffs_df_speckle[col].str.replace('i', 'j').apply(complex)

psi_coeffs_vals = psi_coeffs_df_speckle.iloc[sample_idx,:].values
...
fig = plt.figure(figsize=(30, 8))
plt.plot(x_range, np.absolute(sample_signal_vals_speckle[1,:]))
plt.title("Sample Input Signal (Speckle 0.6)")
...
image_object = Image(x_range, window_func=rect_window, signal=sample_signal_vals_speckle, psi_obj=rec_fourier_psi, F=F)

image_integral = image_object._evaluate_image()

fig = plt.figure(figsize=(30, 8))

plt.plot(x_range, true_scatterers_speckle, 'orange', lw=3)
...
model_output = run_inference(model, params, split_complex_to_imaginary(sample_signal_vals_speckle[1,:]))
...
model_image_object = Image(x_range, window_func=rect_window, signal=sample_signal_vals_speckle, psi_obj=model_rec_fourier_psi, F=F)
...
plt.title("Image Integral (NN) Inference on Higher Speckle Data")
plt.legend(["True Point Scatterers (Speckle 0.6)", "Image Integral  (NN)"])
plt.savefig('neural_image_integral_more_speckle.png', dpi=300)
...
plt.savefig('neural_image_integral_combined_more_speckle.png', dpi=300)
...
plt.xlabel("x")
plt.ylabel("Psi")
plt.title("Psi Reconstruction (Trained on Lower Speckle)")
plt.grid(True)
plt.legend()
plt.show()
plt.savefig('psi_comparison_more_speckle.png', dpi=300)
