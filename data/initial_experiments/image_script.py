...
islr_avg = compute_islr(np.absolute(image_integral)/dx, true_scatterers, x_range, 5, dx)
print(islr_avg)
...
for i in range(10):
    ...
    signal_df_relnoise = pd.read_csv(signal_path).map(convert_to_complex).T.iloc[sample_idx,:].values
    ...
    true_scatterers_relnoise = pd.read_csv(scatterer_path).map(convert_to_complex).iloc[:,sample_idx].map(np.absolute).values
