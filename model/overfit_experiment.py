...
label_df = pd.read_csv(config['paths']['label_data_file_path'], dtype=str).iloc[:,:NUM_DATA_POINTS]
data_df = pd.read_csv(config['paths']['signal_data_file_path'], dtype=str).iloc[:NUM_DATA_POINTS,:]
...
data_array = data_df.map(convert_to_complex).to_numpy()
data_matrix = normalize_complex_to_unit_range(data_array)
...
test_label_df = pd.read_csv(config['paths']['test_label_file_path'], dtype=str).iloc[:,:NUM_DATA_POINTS]
test_data_df = pd.read_csv(config['paths']['test_data_file_path'], dtype=str).iloc[:NUM_DATA_POINTS,:]
...
def loss_fn(params, model, inputs, true_coeffs, deterministic, rng_key,
    ...
    #return (fourier_weight * direct_loss + fourier_d1_weight * d1_loss +
            #fourier_d2_weight * d2_loss +
            #l2_reg_weight * l2_loss +
            #l4_weight * loss_l4 +
            #0 * amplitude_loss)
    return (fourier_weight * direct_loss + l2_reg_weight * l2_loss + l4_weight* loss_l4), direct_loss
...
def train_step(state, batch_signal, batch_coeffs, rng_key):
    def loss_wrapper(params):
        total, direct =  loss_fn(params, model, batch_signal, batch_coeffs, False, rng_key,
                       ionoNHarm, kpsi_values, False,
                       l2_reg_weight, fourier_weight, fourier_d1_weight, fourier_d2_weight, l4_weight,
                       F, dx, xi, zero_pad)
        return total,direct
    (loss, direct_loss), grads = jax.value_and_grad(loss_wrapper, has_aux=True)(state.params)
    return state.apply_gradients(grads=grads), loss, direct_loss
...
flattened_input = data_matrix_split.reshape(data_matrix_split.shape[0], -1)
dummy_input = flattened_input
...
with jax.disable_jit():  # optional: makes debugging easier
    init_preds = model.apply({'params': state.params}, data_matrix_split, deterministic=True)
    preds_real, preds_imag = init_preds[:, :6], init_preds[:, 6:]
    print("Initial preds (real):", preds_real)
    print("Initial preds (imag):", preds_imag)
    print("Initial diff (sample 0 vs 1):", jnp.abs(preds_real[0] - preds_real[1]))
...
with open("training_losses_color.csv", "w", newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["Epoch", "Training Loss", "Test Loss", "Training Direct Loss", "Test Direct Loss"])
...
for epoch in tqdm(range(config['optimizer']['maxiter_adam']), desc="Training"): # just defines a progess bar to print so i can keep an eye on it
    ...
    for batch_signal, batch_coeffs in data_loader(*dataset, config['training']['batch_size']):
        ...
        state, loss, direct_loss = train_step(state, batch_signal, batch_coeffs, subkey)
        batch_loss += loss
        batch_fourier_loss += direct_loss
    # calculate the average loss for the batch, and then save it out
    avg_epoch_loss = batch_loss / num_batches
    avg_fourier_loss = batch_fourier_loss / num_batches
    ...
    for test_signal, test_coeffs in data_loader(*test_dataset, config['training']['batch_size'], shuffle=False):
        test_batch_loss, direct_loss = loss_fn(
            state.params, model, test_signal, test_coeffs,
            True, rng_key, ionoNHarm, kpsi_values, True,
            l2_reg_weight, fourier_weight, fourier_d1_weight, fourier_d2_weight, l4_weight,
            F, dx, xi, zero_pad)
        test_loss += test_batch_loss
        test_fourier_loss += direct_loss

    avg_test_loss = test_loss / test_batches
    avg_test_fourier_loss = test_fourier_loss / test_batches
    ...
    with open("training_losses_color_smaller.csv", "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow([epoch + 1, avg_epoch_loss, avg_test_loss, avg_fourier_loss, avg_test_fourier_loss])

    with jax.disable_jit():
        preds = model.apply({'params': state.params}, data_matrix_split, deterministic=True)
        diff = jnp.mean(jnp.abs(preds[0] - preds[1]))
        print(f"Epoch {epoch+1} mean pred diff: {diff}")
...
final_weights_name = f"model_weights_color_overfit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
